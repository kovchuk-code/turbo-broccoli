...
def add_page_numbers(input_pdf, output_pdf, start_page=1, start_number=1, 
    ...
    # Для Times New Roman используем встроенный шрифт times
    # Встроенные шрифты PyMuPDF: helv (Helvetica), times (Times Roman), cour (Courier)
    font_to_use = internal_font
    ...
    print(f"  - Шрифт: {font_name} (внутренний: {font_to_use})")
    print(f"  - Размер шрифта: {font_size} pt")
    print(f"  - Смещение по X: {offset_x} pt")
    print(f"  - Смещение по Y: {offset_y} pt")
    print(f"  - Примечание: Для Times New Roman используется встроенный шрифт 'times'")
    ...
    for page_num in range(total_pages):
        ...
        if page_num + 1 >= start_page:
            ...
            text_width = fitz.get_text_length(text, fontname=font_to_use, fontsize=font_size)
...
class NumberPagesTab:
    ...
    def create_widgets(self):
        ...
        # Настройка: выбор шрифта
        ttk.Label(settings_frame, text="Шрифт:", 
        ...
        font_combobox = ttk.Combobox(
            ...
            values=["Arial", "Times New Roman", "Isocpeur"],
            width=20,
            font=('Arial', 10),
            state="readonly"
        )
        font_combobox.grid(row=3, column=1, padx=10, pady=10, sticky="w")
        
        ttk.Label(settings_frame, text="(Times New Roman - встроенный шрифт)", 
                 font=('Arial', 9), foreground='gray').grid(row=3, column=2, padx=5, pady=10, sticky="w")
    ...
    def update_file_info(self, event=None):
        ...
        if input_file and os.path.exists(input_file) and input_file.lower().endswith('.pdf'):
            try:
                ...
                # Корректируем значение start_page, если оно больше total_pages
                current_start_page = self.start_page_var.get()
                if current_start_page > total_pages:
                    self.start_page_var.set(total_pages)
    ...
    def process_pdf(self):
        ...
        try:
            add_page_numbers(input_file, output_file, start_page, start_number, use_background, font_name)
